define([
  ...
  'dijit/form/Select'
    ...
    Query, QueryTask,
    GraphicsLayer, webMercatorUtils, arcgisUtils,JSON,
    lang, html, array,
    mouse, on, Memory,ObjectStore,
    topic){
  return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin],{
    baseClass: 'context-save-as-view',
    ...
    _search: function() {
        topic.publish("/widgets/FromContext/action/switchTab", null, { tab: this.nls.results });
        
        var query = new Query();
        query.queryUrl = this.inputQueryCatalog.value;
        query.start=1;
		
		// if the registered catalog inlcudes RID (geoportal server federates to remote catalog)
		// move the rid from the queryUrl to an attribute of query object.
		// sample: http://gptogc.esri.com/geoportal/rest/find/document?rid=dataGov
		if (query.queryUrl.indexOf("?rid=")>0) {
			var queryUrlPieces = query.queryUrl.split("?rid=");
			query.queryUrl = queryUrlPieces[0];
			query.rid = queryUrlPieces[1];
		} else {
			query.rid = "";
		}
