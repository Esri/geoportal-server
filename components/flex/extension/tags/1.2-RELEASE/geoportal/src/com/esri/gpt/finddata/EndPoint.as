...
public class EndPoint {
  ...
  // constructors ==============================================================
  public function EndPoint(siteName:String, siteUrl:String, 
    ...
    var url:String;
	  for each (var endPoint:XML in endPointList) {
	    ...
	    if(sRegEndpointList != "" && 
		  ...
		  endPoints.addItem(new EndPoint(endPoint.@name, 
  ...
  * */
  ...
  /**
		 ...
		 * */
	public function extractServiceType(url:String):EnumServiceTypes {
    var serviceType:EnumServiceTypes  = null;
    if(url == null || url == "") {
  	  return null;
    }
    //url = url.toLowerCase();
    ...
    var wfsReg:String = 
      _configXml.linkParser.(@id == linkParserId).regLinkTypes.wfs.regDetect;
    var agsMapServerReg:String = 
      _configXml.linkParser.(@id == linkParserId).regLinkTypes
        .agsRestMapServer.regDetect;
    var agsFeatureServerReg:String = 
      _configXml.linkParser.(@id == linkParserId).regLinkTypes
        .agsRestFeatureServer.regDetect;
    var agsImageServerReg:String = 
      _configXml.linkParser.(@id == linkParserId).regLinkTypes
        .agsRestImageServer.regDetect;
	var kmlReg:String = 
		_configXml.linkParser.(@id == linkParserId).regLinkTypes
		.kml.regDetect;
	var agsWebmapReg:String = 
		_configXml.linkParser.(@id == linkParserId).regLinkTypes
		.agsWebmap.regDetect;
    ...
    var arrWfs:Array = url.match(new RegExp(wfsReg));
    var arrMapServer:Array = url.match(new RegExp(agsMapServerReg));
    var arrFeatureServer:Array = url.match(new RegExp(agsFeatureServerReg));
    var arrImageServer:Array = url.match(new RegExp(agsImageServerReg));
	var arrKml:Array = url.match(new RegExp(kmlReg));
	var arrAgsWebMap:Array = url.match(new RegExp(agsWebmapReg));
	
    if (Utils.chkStringMatch(arrWms).length >= 1 ) {
      serviceType = EnumServiceTypes.WMS;
    } 
	
	else if (Utils.chkStringMatch(arrWfs).length >= 1) {
      serviceType = EnumServiceTypes.WFS;
    } else if (Utils.chkStringMatch(arrMapServer).length >= 1) {
      serviceType = EnumServiceTypes.AGS_REST_MAPSERVER;
    } else if (Utils.chkStringMatch(arrFeatureServer).length >= 1) {
      serviceType = EnumServiceTypes.AGS_REST_FEATURESERVER;
    } else if (Utils.chkStringMatch(arrImageServer).length >= 1) {
      serviceType = EnumServiceTypes.AGS_REST_IMAGESERVER;
    } else if (Utils.chkStringMatch(arrKml).length >= 1) {
		serviceType = EnumServiceTypes.KML;
	} else if (Utils.chkStringMatch(arrAgsWebMap).length >= 1) {
		serviceType = EnumServiceTypes.AGS_WEBMAP;
	} else if (Utils.chkStringMatch(arrIms).length >= 1) {
		serviceType = EnumServiceTypes.AIMS;
	}
  ...
  */
  public function clone():EndPoint {
    return new EndPoint(this.siteName,this.siteUrl, 
  ...
  * */
  ...
  /**
  * Calculates the geoportal url
  * */
  public static function calculateGeoportalUrl():String {
    var url:String = ExternalInterface.call("window.location.href.toString");
    var arrLoaderUrl:Array = url.split("?");
    var loaderUrl:String = arrLoaderUrl[0];
    loaderUrl = loaderUrl.replace(/[^\/]*$/, "../");
   // Alert.show( "gptURL = " + loaderUrl);
    return loaderUrl;
