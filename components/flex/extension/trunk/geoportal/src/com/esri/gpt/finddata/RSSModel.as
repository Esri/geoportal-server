  ...
  * */  
	public class RSSModel {
	  ...
	  private var _dctMapsBeingAdded:Dictionary = new Dictionary();
		...
		// constructors ===================================================
		public function RSSModel(configXml:XML) {
		  if(configXml == null) {
		    return;
		  }
    ...
    * */
    public function readBbox(bbox:String, spatialRef:SpatialReference):Polygon {
	  ...
	  * */
	  public function getGeomCenter(geom:Geometry):MapPoint {
		 ...
		 * */
		public function readService(record:SearchResultRecord, listener:Function, 
		  ...
		  for each (var objLink:Object in record.links) {
		    var link:EndPoint = objLink as EndPoint;
        if(link == null) {
          continue;
        }
        ...
        try {
          ...
          sServiceUrl = EndPoint.readString(regExtractMapservice, 
            sServiceUrl);
          ...
          if(Utils.chkString(sServiceUrl) != "" ) {
            ...
            sServiceType = EndPoint.readString(regExtractMapLinkType, 
              sServiceType);
            
            // Attempting to find service type object
            var enumServiceType:EnumServiceTypes = 
	 ...
	 * 
	 * */	
	 public function removeExcludedLinks(record:SearchResultRecord,
	   endPoint:EndPoint):ArrayCollection {
	   ...
	   for each (var objLink:Object in record.links) {
	     var endPoint:EndPoint = objLink as EndPoint;
	     if(endPoint == null) {
	       continue;
	     }
		  ...
		  try {
		    if(bDebug) {
			  ...
			  if (event.result is XML) {
				  ...
				  var rssObj:RSSParser = RSSParser.createRssParser(
    ...
    * */
    public function createUrlCriteria(criteria:RSSCriteria, 
      url:String):String {
      ...
      if (criteria.extentOption == RSSCriteria.EXTENT_INTERSECT) {
        params["spatialRel"] = "esriSpatialRelOverlaps";
        bIncludeBbox = true;
      } else if (criteria.extentOption == RSSCriteria.EXTENT_CONTAINS) {
