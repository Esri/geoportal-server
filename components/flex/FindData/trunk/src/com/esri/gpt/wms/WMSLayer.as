...
package com.esri.gpt.wms
{
	 ...
	 */
	public class WMSLayer extends com.esri.ags.layers.WMSLayer
	{
	  ...
	  private static const LOG:ILogger = Log.getLogger("WMSError");
				...
				function onResult( event:ResultEvent, token:Object = null ):void {
				  ...
				  if(error != null && error != "") {
				    dispatchEvent(new LayerEvent(LayerEvent.LOAD_ERROR, 
				      event.token.layer, new Fault("WMS",error,error)));
				  } else {
				    // Load the image URL
		...
		**/
		...
		override public function set visibleLayers(arr:IList):void {
		  ...
		  this._visibleLayers = arr as ArrayCollection;
