...
namespace com.esri.gpt.publish
{
    ...
    public class PublicationRequest : IPublicationRequest
    {
        ...
        /// <summary>
        /// Reads translator path metadata style config file. 
        /// </summary>
        /// <remarks>
        /// used to get translator path for currently selected metadata style.
        /// </remarks>
        /// <param name="filePath">Currently selected metadata style config file path.</param>
        /// <returns>Returns translator path for metadata configuration.</returns>
        private String readTranslatorFromConfig(string filePath)
        {
            try
            {
                XmlDocument doc = new XmlDocument();
                doc.Load(filePath);
                XmlNodeList xmlnodes = doc.GetElementsByTagName("translator");
                XmlNode translatorNd = xmlnodes.Item(0);
                return translatorNd.InnerText;
            }
            catch (Exception ex)
            {
        ...
        //    string endPoint = determineEndPoint();
            ...
            if (endPoint == "REST" || endPoint == "DEFAULT")
            {
                ...
                // send untransformed metadata xml if transformed metadata xml was unrecognized
                // by Geoportal Server.
                if (resp.Contains("409") && (resp.ToLower().Contains("unrecognized metadata schema.")))
                {
                    sXml = File.ReadAllText(xmlFilePaths[0], Encoding.UTF8);
                    resp = SubmitHttpRequest(sXml, sUrl);
                }

                if ((resp.ToLower().Contains("error") || resp.ToLower().Contains("exception")) && !resp.Contains("409") && !resp.Contains("Invalid metadata document."))
                {                
