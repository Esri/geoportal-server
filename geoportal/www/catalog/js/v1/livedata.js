...
gpt.LiveData.setExtent = function(map,extent,gs) {
  if (extent!=null) {
    if (map.spatialReference.wkid!=extent.spatialReference.wkid) {
      if (gs!=null) {
        var gr = new esri.Graphic(extent);
        gs.project([gr], map.spatialReference, function(gfx) {
          map.setExtent(gfx[0].geometry);
        });
      }
    } else {
...
dojo.provide("esri.layers.ArcIMSLayer");

/** ArcIMS Layer **/
dojo.declare("esri.layers.ArcIMSLayer", esri.layers.DynamicMapServiceLayer, {
  _proxyUrl: "",
  _url : "",
  _imageUrl: "",
...
dojo.provide("esri.layers.WMSLayer");
...
dojo.provide("esri.layers.GeorssLayer");

/** GEORSS layer **/
dojo.declare("esri.layers.GeorssLayer", esri.layers.GraphicsLayer, {
  _proxyUrl: "",
  _url : "",
  _gs : null,
  _map : null,
  ...
  _addToMap: function(graphics,infoTemplate) {
    try {
      if (this._gs!=null && this._map!=null && graphics.geometry.spatialReference.wkid!=this._map.spatialReference.wkid) {
        this._gs.project([ graphics ], this._map.spatialReference, dojo.hitch(this, function(gfx) {
          graphics.setGeometry(gfx[0].geometry);
          graphics.setInfoTemplate(infoTemplate);
          this._map.graphics.add(graphics);
        }));
      } else {
        graphics.setInfoTemplate(infoTemplate);
        this._map.graphics.add(graphics);
  ...
  _drawSinglePointByCoords: function(lon,lat,markerSymbol,infoTemplate) {
      ...
      this._addToMap(graphic, infoTemplate);
  },
...
/** KML Layer **/
dojo.declare("esri.layers.KmlLayer", esri.layers.GraphicsLayer, {
  _proxyUrl: "",
  _url : "",
  _gs : null,
  _map : null,
...
/** SOS Layer **/
dojo.declare("esri.layers.SOSLayer", esri.layers.GraphicsLayer, {
  _contextUrl: "",
  _proxyUrl: "",
  _url : "",
  _gs : null,
  ...
  _addPoint: function(point, markerSymbol, caption, content) {
    ...
    var graphics = new esri.Graphic(point, markerSymbol);
    this._addToMap(graphics, infoTemplate);
  },
