...
dojo.declare("GptMap", null, {
  ...
  initialize: function(config) {
    ...
    if ((config.mapServiceURL != null) && (config.mapServiceType != null)) {
...
//    this._agsMap.addLayer(service);
  ...
  getWorldExtent: function getWorldExtent() {
    ...
    return new esri.geometry.Extent(-180,-90,180,90,ref);
  },
  ...
  zoomToGCSExtent: function(extent,bExpand) {
    ...
    if (env != null) {
      ...
      if (env.xmin > env.xmax) {
        var ref = new esri.SpatialReference({wkid:4326});
        env = new esri.geometry.Extent(-180.0,env.ymin,180.0,env.ymax,ref);
      }
    }
  ...
  highlightInputEnvelope: function() {
    if (!this._isHidden && (this._gptMap != null)) {
      ...
      if (env != null) { 
        ...
        if (env.xmin > env.xmax) {
          var ref = new esri.SpatialReference({wkid:4326});
          var envLeft = new esri.geometry.Extent(-180.0,env.ymin,env.xmax,env.ymax,ref);
          var envRight = new esri.geometry.Extent(env.xmin,env.ymin,180.0,env.ymax,ref);
