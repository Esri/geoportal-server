      ...
      this.onExtractData = function onExtractData() {
        ...
        for(i=0 ; i < _layersCount ; i++) {
          ...
          if (currentLayer!=null && currentLayer.checked) {
            ...
            checkedLayers += currentLayer.name;
          }
        }

        // Get output format
        var selObj = document.getElementById('selOutputFormat');
        var selIndex = selObj.selectedIndex;
        var outFormat = selObj.options[selIndex].value;

        // Get projection
        var selObj2 = document.getElementById("selProjection");
        var selIndex2= selObj2.selectedIndex;
        var outProjection = selObj2.options[selIndex2].value;
        ...
        if (userEmail.length == 0) {
          ...
          var params = {"Output_coordinate_system": outProjection, "Output_Format": outFormat, "Area_To_Download": _featureSet, "Layers_to_Clip": checkedLayers , "Email_address_to_send_zip_file": userEmail  };
...
<h:panelGrid columns="1"
  ...
  <h:column>
    ...
    <h:panelGrid id="pgdCriteria" columns="2" columnClasses="formLabelColumn,formInputColumn">
      ...
      <h:outputLabel for="selOutputFormat"
                     value="#{gptMsg['catalog.download.outputFileFormat']}" />
      <h:selectOneMenu id="selOutputFormat"
                       value="#{DownloadController.selectedOutputFormat}">
        <f:selectItems value="#{DownloadController.allOutputFormats}" />
      </h:selectOneMenu>

      <h:outputLabel for="selProjection"
                     value="#{gptMsg['catalog.download.projection']}" />
      <h:selectOneMenu id="selProjection"
                       value="#{DownloadController.selectedProjection}">
        <f:selectItems value="#{DownloadController.allProjections}" />
      </h:selectOneMenu>
