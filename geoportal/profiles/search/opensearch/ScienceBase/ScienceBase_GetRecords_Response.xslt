		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<Records>
					...
					<xsl:for-each select="/gptJsonXml/items">
						<Record>
							<ID>
							...
							<Abstract>
								<xsl:value-of select="normalize-space(./summary/text())"/>
							</Abstract>
							<Type>
...
-->
							...
							<Links>  
                 <Link label="catalog.search.searchResult.openResource"><xsl:value-of select="normalize-space(./link/url)"/></Link>                      
