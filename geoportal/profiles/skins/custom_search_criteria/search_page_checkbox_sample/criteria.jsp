...
<f:verbatim>
  ...
  function scReadRestUrlParams() {
    ...
    dojo.byId('frmSearchCriteria:scHierarchy').value);
    if(scHierarchy != "") {
    ...
    while(true) {
        ...
        var el = dojo.byId('frmSearchCriteria:scSelHierachy:' +  i);
        if(el == null) {
            break;
        } else if(dojo.attr(el, 'checked')) {
            
            var elVal  = dojo.attr(el, 'value');
            if(elVal == null || elVal == '') {
                continue;
            }
            if(restHcy != null && restHcy != '') {
                restHcy  += " OR ";
            } 
            restHcy += dojo.trim(elVal);
        } 
        i++;
    }
    if(restHcy != null && restHcy != '') {
        restParams += "&hierarchy=" + encodeURIComponent(restHcy)+"&";
    }
...
<h:outputText escape="false" value="</h3>"/>
<h:selectManyCheckbox id="scSelHierachy" layout="pageDirection"
                          value="#{SearchFilterHierarchy.selectedHierachy}">            
        <f:selectItem
        itemLabel="#{gptMsg['catalog.search.filterHierarchy.dataset']}"
        itemValue="dataset" />
          <f:selectItem
        itemLabel="#{gptMsg['catalog.search.filterHierarchy.feature']}"
        itemValue="feature" />
          <f:selectItem
        itemLabel="#{gptMsg['catalog.search.filterHierarchy.model']}"
        itemValue="model" />
          <f:selectItem
        itemLabel="#{gptMsg['catalog.search.filterHierarchy.series']}"
        itemValue="series" />
          <f:selectItem
        itemLabel="#{gptMsg['catalog.search.filterHierarchy.service']}"
        itemValue="service" />
          <f:selectItem
        itemLabel="#{gptMsg['catalog.search.filterHierarchy.software']}"
        itemValue="software" />
          <f:selectItem
        itemLabel="#{gptMsg['catalog.search.filterHierarchy.tile']}"
        itemValue="tile" />          
