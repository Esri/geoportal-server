...
/**
 * The Class SearchController. Controller for search operations. To be used in
 * JSF scope.
 */
...
//instance variables ==========================================================
  ...
  /**
   * Flag indicating whether or not result records should be expanded by
   * default.
   */
...
// properties  =================================================================
   ...
   *   * 
public ListDataModel getResultRecordsAsListModel() { ListDataModel model =
   * new ListDataModel();
   * model.setWrappedData(this.getSearchResult().getRecords()); return model; }
   */
  /**
...
// methods =====================================================================
  ...
  /**
   * Does process request parameters. It is used to process 'uuid' parameter to
    ...
    try {
      ...
      if (parameterMap.containsKey("uuid")) {
        ...
        if (oUuid instanceof String[] && ((String[]) oUuid).length > 0) {
          doViewMetadataDetails(context, ((String[]) oUuid)[0], catalogUrl, cswContext);
        }
      }
  ...
  protected void processSearchActions(ActionEvent event, RequestContext context)
    ...
    if (eventType == null) {
      ...
      throw new SearchException("Controller could not determine type of event passed");
     ...
     doViewMetadataDetails(context,uuid);
      ...
      try {
        FacesContextBroker facesBroker = new FacesContextBroker();
        Map requestMap = facesBroker.getExternalContext().getRequestParameterMap();
        String searchId = (String) requestMap.get(SearchEvents.Event.PARAM_UUID);
        doLoad(searchId);
        success = true;
  ...
  /**
   * Redoes the search by using the pagecursor in the session. Looks for
   * parameter SearchEvents.Event.PARAM_UUID
   *
   * @param event the event
  ...
  private void redoSearch(ActionEvent event,
    ...
    String dosearch = (String) requestMap.get(SearchEvents.Event.PARAM_EXECUTE_SEARCH);
    if ("".equals(Val.chkStr(dosearch)) && event != null
      && event.getComponent() != null) {
      UIComponent component = event.getComponent();
      dosearch = (String) component.getAttributes().get(
   ...
   * Do search.
   *
   * @param page the page number (if page <= 0, last page in session will be
   * used) @param doPrefetch the do prefetch
   *
   ...
   * @throws ParserConfigurationException if parser cofiguration exception
   * occurs while loading details style page
   * @throws IOException if I/O exception occurs which reading details style
   * page
   * @throws XPathExpressionException
   */
  private void doViewMetadataDetails(RequestContext context, String uuid, String catalogUri,CswContext cswContext)
    ...
    String metadataXml = this.getMetadataText(uuid, catalogUri,cswContext);
   ...
   *
   * @throws SearchException the search exception
   * @deprecated replaced with  {@link #getMetadataText(String,String,CswContext)}
   */
  @Deprecated
  public String getMetadataText(String uuid, String catalogUri)
    ...
    try {
      ...
      throw new SearchException("Invalid Search URL given for catalog " + catalogUri + " "
        + e.getMessage(), e);
    }
   ...
   * @throws SearchException the search exception
   */
  public String getMetadataText(String uuid, String catalogUri, CswContext cswContext)
    throws SearchException {
    if (uuid == null || "".equals(uuid)) {
      throw new SearchException("UUID given for document requested is either null"
        + " or empty");
    }
    ASearchEngine dao = cswContext!=null? getSearchDao(cswContext): getSearchDao();
   ...
   * @throws SearchException the search exception
   */
  protected ASearchEngine getSearchDao(CswContext cswContext) throws SearchException {
    ASearchEngine dao = SearchEngineFactory.createSearchEngine(
