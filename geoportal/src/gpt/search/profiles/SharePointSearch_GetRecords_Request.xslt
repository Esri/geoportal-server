...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
	  ...
	  <xsl:choose>
         ...
         <xsl:text>&amp;k=thereShouldBeNoDataThatMatchesThisDefaultString</xsl:text>
	     </xsl:when>
	     <xsl:when test="$keyword != '' and $bLiveData = 'TRUE'">
         <xsl:text>&amp;k=</xsl:text><xsl:value-of select="$keyword"/>
         <xsl:text>%20typekeywords:service</xsl:text>
       </xsl:when>
       <xsl:when test="$bLiveData = 'TRUE'">
         <xsl:text>&amp;k=typekeywords:service</xsl:text>
       </xsl:when>
        <xsl:otherwise>
